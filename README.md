...
- **To get started,** clone the plugin repository:
  ```sh
  git clone https://github.com/nwintersgill/IntelliJ_licensing_plugin
  ```
...
2. Add a new Gradle run configuration:\
    **Gradle task: runIde**
3. Click Run ▶️\
    IntelliJ will start a sandboxed IDE with your plugin installed.
4. Open a Maven project in the sandboxed IDE to see the plugin in action. The chat window should open automatically, and can be opened or closed with a button on the right side of the screen.
5. Use the dropdown at the bottom of the chat window to select a model. Note that the chatbot will not function if you select a model which is not configured.
6. In the top right corner of the chat window, click the "Survey" button to enter licensing information about your repository. This will be used as context for the model. 
7. In the bottom right corner of the chat window, click the "API Key" button to enter your OpenAI API key, if you wish to use OpenAI models.
